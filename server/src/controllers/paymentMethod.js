...
export async function getPaymentMethods(req, res) {
  try {
    const [rows] = await db.query("SELECT * FROM payment_methods ORDER BY method_id");
...
export async function getPaymentMethodById(req, res) {
  try {
    const [rows] = await db.query("SELECT * FROM payment_methods WHERE method_id=?", [req.params.id]);
    if (rows.length === 0)
...
export async function updatePaymentMethod(req, res) {
  try {
    ...
    await db.query(
      "UPDATE payment_methods SET method_name=?, description=? WHERE method_id=?",
...
export async function deletePaymentMethod(req, res) {
  try {
    await db.query("DELETE FROM payment_methods WHERE method_id=?", [req.params.id]);
    res.json({ success: true });
  } catch (err) {
    console.error(err);
